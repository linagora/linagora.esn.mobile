...
describe('The Push sender module', () => {
  ...
  describe.skip('The sendToSubscriptions function', () => {
    it('should send subscription to all subscriptions which have a valid transport', (done) => {
      let gcmSpy = sinon.spy();
      let apnsSpy = sinon.spy();
      let firebaseSpy = sinon.spy();
      let libMocks = {
        pushsubscription: {},
        application: {}
      };
      ...
      mockery.registerMock('./transport/firebase', function() {
        return {send: firebaseSpy};
      });

      let module = getModule(function() {}, libMocks);
      module.sendToSubscriptions(application, message, subscriptions).then((result) => {
